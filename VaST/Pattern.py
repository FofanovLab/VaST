...
class Patterns:
    ...
    def to_json(self, file_name, strains):
        d = {"strains": strains,
            "genomes": {v: k for k, v in self._genomes.iteritems()},
    ...
    def add_new_pattern(self, pattern, amplicon):
        pattern_str = self._pattern_to_string(pattern)
        if amplicon.genome not in self._genomes:
            self._genomes[amplicon.genome] = len(self._genomes)
        ...
        if pattern_str in self._patterns:
            ...
            self._patterns[pattern_str] = {amplicon.start: {'s': amplicon.stop,
                                                            'g': self._genomes[amplicon.genome],
                                                            'sites': amplicon.site_ids}}
    def add_ambiguous_amplicon(self, features, amplicon):
        features = [[tuple(r) for r in cartesian_product(row)] for row in features]
        ...
        self.add_new_pattern(pattern, amplicon)


    def add_unambiguous_amplicon(self, features, amplicon):
        # Requires numpy >= 1.13
    ...
    # def _ambiguous_pattern(self, feature, feature_categories):
    #     pattern = []
